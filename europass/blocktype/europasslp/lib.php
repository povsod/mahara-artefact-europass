...
class PluginBlocktypeEuropassLP extends PluginBlocktype {
    ...
    public static function render_instance(BlockInstance $instance, $editing=false) {
		...
		//$xmlDoc->loadXML(generate_europasslp_xml((!empty($configdata['userid']) ? $configdata['userid'] : $USER->get('id')), true, $locale, (!empty($configdata['internaldate']) ? $configdata['internaldate'] : 'dmy11'), (!empty($configdata['externaldate']) ? $configdata['externaldate'] : '/numeric/long')));
		$xmlDoc = simplexml_load_string(generate_europasslp_xml((!empty($configdata['userid']) ? $configdata['userid'] : $USER->get('id')), true, $locale, (!empty($configdata['internaldate']) ? $configdata['internaldate'] : 'dmy11'), (!empty($configdata['externaldate']) ? $configdata['externaldate'] : '/numeric/long')));
		...
		$xslDoc = simplexml_load_string(file_get_contents(get_config('wwwroot') . 'artefact/europass/blocktype/europasslp/xsl/lp_' . $locale . '_V2.0.xsl'));
		...
		$local_imgpath = get_config('wwwroot') . 'artefact/europass/images/';
		$europass_imgpath = 'http://europass.cedefop.europa.eu/instruments/images/logos/';
		$result = str_replace($europass_imgpath, $local_imgpath, $result);
    ...
    public static function artefactchooser_element($default=null) {
        return array(
            'name'  => 'locale',
            'type'  => 'select',
            'title' => get_string('showinlocale', 'blocktype.europass/europasslp'),
